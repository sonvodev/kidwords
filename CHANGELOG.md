...
### Fixed
- **No stray audio on entering Learn**: the first word was briefly read aloud
  before the 3-2-1 countdown due to an effect firing while `countdown` was still
  unset. Reading is now gated on active playback, so it only starts after the
  countdown.
...
[1.4.3]: https://github.com/sonvodev/kidwords/releases/tag/v1.4.3
